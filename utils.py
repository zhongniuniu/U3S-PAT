...
from data import ImageDataset, ImageDataset_2D, ImageDataset_ISS_2D, ImageDataset_3D
...
def get_ISS_data_loader(data, img_path, img_dim, img_slice,
                    train, batch_size,
                    num_workers=4,
    ...
    if data == 'phantom':
        ...
        dataset = ImageDataset_ISS_2D(img_path, img_dim, img_slice)
...
def ct_parallel_project_2d(img, theta):
    ...
    # Rotation transform matrix: simulate parallel projection rays 旋转变换矩阵:模拟平行投影射线
    ...
    # Resample (x, y) index of the pixel on the projection ray-theta 重新采样投影射线上像素的(x, y)索引
